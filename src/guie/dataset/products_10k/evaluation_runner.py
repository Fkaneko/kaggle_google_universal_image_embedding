...
from .build_dataset import dataset_pipeline, get_dataloader, load_products_10k_dataset
from .constants import products_10k_const

logger = logging.getLogger(__name__)


def prepare_products_10k_eval_input(
    ...
    products_10k_dataset, solution_index = load_products_10k_dataset(
        data_dir=data_dir,
    ...
    products_10k_dataset = dataset_pipeline(
        products_10k_dataset=products_10k_dataset,
        data_dir=data_dir,
        image_transformations=image_transformations,
    )
    logger.info(f"products_10k evaluation dataset: {products_10k_dataset}")
    index_image_dataloader = get_dataloader(
        ds=products_10k_dataset[products_10k_const.INDEX],
        batch_size=batch_size,
        num_workers=num_workers,
    )
    test_image_dataloader = get_dataloader(
        ds=products_10k_dataset[products_10k_const.QUERY],
        batch_size=batch_size,
        num_workers=num_workers,
    )
...
def eval_products_10k(
    ...
    index_image_dataloader, test_image_dataloader, solution_index = prepare_products_10k_eval_input(
        arrow_dir=arrow_dir,
        batch_size=batch_size,
        data_dir=data_dir,
        image_transformations=image_transformations,
    ...
    predictions = generate_retrieval_predictions(
        ...
        image_id_key=products_10k_const.IMAGE_ID,
    ...
    logger.info(f"products_10k: mean ap: {mean_ap}")
    return mean_ap
